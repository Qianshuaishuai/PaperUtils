...
const (
	BASE_URL    = "/home/dashuai/readboyPaper/zhixuewang初中/政治/"
...
func writeSql() {
	...
	for g := range Grades {
		sem = Sems[g]
		grade = Grades[g]
		rd, _ := ioutil.ReadDir("/home/dashuai/readboyPaper/zhixuewang初中/政治/" + Grades[g])
		for r := range rd {
			writePaperSql(rd[r].Name())
		}
	}
...
// 	dbErr := tx.Table("t_papers").Where("F_name = ?", fName).Update("F_date", dateTimeStr)
// 	beego.Debug(dbErr)
// }
...
func writePaperSql(name string) {
	...
	paperProvince.ProvinceID = translateProvinceIDForReadboy(test.Paper.DefaultArea.Name)
	tx.Table("t_paper_province").Create(&paperProvince)
	...
	for m := range questionList {
		if len(questionList[m].SubQuestion) > 1 {
			bigQusetionID, bigQuestionScore, questionCount, difficulty := newBigQuestion(questionList[m], tx)
			...
			question.Solution = questionList[m].OriginalStruct.AnalysisHTML
			...
			if question.Type == 61 && question.CourseID != 55 {
			...
			tx.Table("t_questions").Create(&question)
			...
			for k := range questionList[m].Knowledges {
				...
				var keyPointIDs []int
				keyPointID := 0
				tx.Table("t_keypoints").Where("F_name = ?", questionList[m].Knowledges[k].Name).Pluck("F_keypoint_id", &keyPointIDs)
				...
				if keyPointID != 0 {
					...
					tx.Table("t_keypoint_question").Create(&keyPointQuestion)
				}
			}
		}
	...
	tx.Table("t_paper_question_sets").Create(&paperSet)
	...
	for c := range chapterList {
		...
		tx.Table("t_paper_question_set_chapters").Create(&paperSetChapters)
	}
	...
	paper.Date = getDateTimeForInt(test.Paper.DateTime / 1000)
	paper.Semester = translateSemesterForReadboy(test.Paper.Grade[0].Name)
	paper.ResourceType = 201
	...
	tx.Table("t_papers").Create(&paper)
	tx.Commit()
}
...
func newBigQuestion(data Question, tx *gorm.DB) (int, float64, int, int) {
	...
	for s := range smallQuestionList {
		...
		} else if data.Section.Name == "选词填空" || data.Section.Name == "完成句子" || data.Section.Name == "选词填空-短文" || data.Section.Name == "补全对话" || data.Section.Name == "阅读填空" {
			// index := s + 1
			question.Content = "<p style='padding:0 30px'>" + dealTexContent(smallQuestionList[s].Stem) + "</p>"
		...
		question.Solution = data.OriginalStruct.AnalysisHTML
		question.SolutionAccessories = "[]"
		...
		for k := range data.Knowledges {
			var keyPointIDs []int
			keyPointID := 0
			tx.Table("t_keypoints").Where("F_name = ?", data.Knowledges[k].Name).Pluck("F_keypoint_id", &keyPointIDs)
	...
	tx.Table("t_large_questions").Create(&largeQuestionSimple)
...
func FixSolutionData() {

	var i = 0
	tx := GetDb().Begin()

	var questions []QuestionSimple

	tx.Table("t_questions").Where("F_course_id = ?", 56).Offset(0).Limit(200000).Find(&questions)

	for q := range questions {
		question := questions[q]

		question.Solution = translateSolution(question.Solution)
		err := tx.Table("t_questions").Where("F_question_id = ?", question.QuestionID).Update(&question).Error
		i = i + 1
		beego.Debug(i)
...
func translateSolution(oldStr string) string {
